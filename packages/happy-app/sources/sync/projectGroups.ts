...
/** The session fields searched by the Messenger-style session list. */
...
/** Narrows a project and its workspaces to rows matching a search query. */
    ...
    const workspaces = project.workspaces
        .map((workspace) => (
            workspace.name?.toLocaleLowerCase().includes(normalizedQuery)
                ? workspace
                : { ...workspace, sessions: workspace.sessions.filter((session) => sessionMatchesQuery(session, normalizedQuery)) }
        ))
        .filter((workspace) => workspace.sessions.length > 0);
