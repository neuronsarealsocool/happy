...
const agentEventSchema = z.discriminatedUnion('type', [z.object({
    ...
    // the reducer to settle the pending message; never rendered as a row.
...
export type NormalizedMessage = ({
    ...
    codexItemId?: string,
    /**
     * Session-protocol turn id (Happy sessions only). Agent rows that share a
     * turn id belong to the same answer; a change of id is a turn boundary even
     * when the user row that started the new turn is missing from this device.
     */
    turn?: string,
    /**
