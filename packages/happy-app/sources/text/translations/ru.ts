...
export const ru: TranslationStructure = {
    ...
    settings: {
        ...
        about: 'О программе',
        aboutFooter: 'AgenticMessenger — мобильное приложение для работы с Codex и Claude Code. Использует сквозное шифрование, все данные аккаунта хранятся только на вашем устройстве. Не связано с Anthropic.',
        whatsNew: 'Что нового',
    ...
    server: {
        ...
        notValidHappyServer: 'Это не валидный сервер AgenticMessenger',
    ...
    sessionInfo: {
        ...
        happySessionIdCopied: 'ID сессии AgenticMessenger скопирован в буфер обмена',
        failedToCopySessionId: 'Не удалось скопировать ID сессии AgenticMessenger',
        happySessionId: 'ID сессии AgenticMessenger',
        ...
        happyHome: 'Домашний каталог AgenticMessenger',
    ...
    components: {
        emptyMainScreen: {
            ...
            installCli: 'Установите AgenticMessenger CLI',
            runIt: 'Запустите его',
    ...
    settingsVoice: {
        ...
        byoDescription: 'Используйте собственного агента ElevenLabs вместо стандартного AgenticMessenger. Подписка не требуется — подключайтесь напрямую через свой аккаунт ElevenLabs. Ваш агент должен определить два клиентских инструмента: messageClaudeCode (отправляет текст агенту кодирования) и processPermissionRequest (разрешает или запрещает использование инструментов). Контекст сессии передаётся через динамическую переменную {{initialConversationContext}}.',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: 'Не настроено',
        customAgentIdDescription: 'Введите ваш ElevenLabs Agent ID. Оставьте пустым, чтобы использовать стандартный AgenticMessenger.',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: 'Прямое подключение',
        bypassTokenSubtitle: 'Пропустить сервер AgenticMessenger, подключиться напрямую к ElevenLabs',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'Терминал запрашивает подключение к вашему аккаунту AgenticMessenger. Это позволит терминалу безопасно отправлять и получать сообщения.',
